...
int main(void) {
    ...
    while (1) {
        // --- 1. 处理输入 ---
		        // 按键输入
        char current_key = 0;
        current_key = s1_key_value_get(s1_key_info);
        if (current_key != last_key_pressed && current_key != SWN) { // SWN代表无按键
             handle_keypad_input(current_key);
        }
        last_key_pressed = current_key;
		
		
        // NFC输入
        if (currentState == STATE_IDLE || currentState == STATE_SHOW_STATS) {
        ...
        else if (currentState == STATE_NFC_READ) { // <-- 新增逻辑块
            unsigned char card_id[4];
            unsigned char card_type[2];
            char display_buf[10];
        ...
        // --- 2. 处理状态机逻辑 ---
        if (loop_tick_counter >= (1000 / LOOP_DELAY_MS)) { // 每秒执行一次
            loop_tick_counter = 0;
            ...
            switch(currentState) {
                ...
                case STATE_LONG_REST:
                    if (remaining_seconds <= 0) start_focus_mode();
                    break;
                case STATE_SNOOZE:
                    if (remaining_seconds <= 0) start_rest_mode(); // 贪睡结束，开始正式休息
                    break;
                default: break;
            }
        }
...
void handle_keypad_input(char key) {
    switch(currentState) {
        case STATE_FOCUS:
            if (key == '*') currentState = STATE_PAUSED;
            else if (key == '#') currentState = STATE_IDLE;
            else if (key == '1') remaining_seconds += 5 * 60;
            else if (key == '2' || key == '3') {
                fan_level = (fan_level % 3) + 1;
                if(fan_level == 1) e2_fan_speed_set(e2_fan_info, FAN_SPEED_LOW);
                if(fan_level == 2) e2_fan_speed_set(e2_fan_info, FAN_SPEED_MEDIUM);
                if(fan_level == 3) e2_fan_speed_set(e2_fan_info, FAN_SPEED_HIGH);
            }
            break;
        case STATE_REST:
        case STATE_LONG_REST:
            if (key == '*') start_focus_mode(); // `*`键在休息时也作为跳过键
            if (key == '0') start_focus_mode(); // 跳过休息
            else if (key == '1') remaining_seconds += 5 * 60;
            else if (key == '#') currentState = STATE_IDLE;
            break;
        case STATE_PAUSED:
            if (key == '*') {
                ...
                if(s7_ir_status_get(s7_ir_info) == 1) pir_absence_counter = 0; // 如果人已在，重置PIR
            }
            break;
        case STATE_IDLE:
        case STATE_SHOW_STATS:
